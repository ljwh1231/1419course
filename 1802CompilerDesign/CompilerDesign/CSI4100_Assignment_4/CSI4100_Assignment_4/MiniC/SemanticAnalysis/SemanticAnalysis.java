...
//*/
	...
	//check if e.type can be assigned to t
	public boolean convert(Type t, Expr e) {
		if(e instanceof EmptyExpr) {
			return true;
		} else if(!(e instanceof ExprSequence)) {
			if(t.Tequal(StdEnvironment.floatType) && e.type.Tequal(StdEnvironment.intType)) {
				e = i2f(e);
			}
			return e.type.AssignableTo(t);
		} else {
			return convert(t, ((ExprSequence)e).lAST) && convert(t, ((ExprSequence)e).rAST);
		}
	}

	// count number of expression
	public int exprNum(Expr e) {
		if(e instanceof EmptyExpr) {
			return 0;
		} else if(!(e instanceof ExprSequence)) {
			return 1;
		} else {
			return exprNum(((ExprSequence)e).lAST) + exprNum(((ExprSequence)e).rAST);
		}
	}
	...
	if (x.astType instanceof ArrayType) {
        ...
        // the lexeme x.astIdent.iexeme together with its declaration x into
	...
	if (!(x.eAST instanceof EmptyExpr)) {
            ...
            if (x.tAST instanceof ArrayType) {
				...
				if (!(x.eAST instanceof	ExprSequence)) {
					...
					if(!convert(typeOfArrayType(x.tAST), x.eAST)) {
						reporter.reportError(errMsg[13], "", x.eAST.pos);
					} else if(exprNum(x.eAST) > ((ArrayType)x.tAST).GetRange()) {
	...
	FunDecl F = (FunDecl ) D;
        ...
        /* Start of your code: */
		int fNum = GetNrOfFormalParams(F);
		int aNum = GetNrOfActualParams(x);
		if(fNum < aNum) {
